...
const SHAPE1:[(f64, f64); 7] = [(79.0, 14.0), (109.0, -17.0), (140.0, 14.0), (124.0, 14.0), (124.0, 70.0), (95.0, 70.0), (95.0, 14.0)];
const SHAPE2:[(f64, f64); 7] = [(59.0, 51.0), (89.0, 20.0), (120.0, 51.0), (104.0, 51.0), (104.0, 107.0), (75.0, 107.0), (75.0, 51.0)];
const SHAPE3:[(f64, f64); 7] = [(39.0, 88.0), (69.0, 57.0), (100.0, 88.0), (84.0, 88.0), (84.0, 144.0), (55.0, 144.0), (55.0, 88.0)];
...
pub fn draw(context: &CanvasRenderingContext2d){
    ...
    /*画图 */
    draw_shape(context, &SHAPE1.iter().map(|point|{ transform(point.0, point.1) }).collect());
    draw_shape(context, &SHAPE2.iter().map(|point|{ transform(point.0, point.1) }).collect());
    draw_shape(context, &SHAPE3.iter().map(|point|{ transform(point.0, point.1) }).collect());
}
